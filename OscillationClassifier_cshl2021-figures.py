...
Whi5_dcategory = pipeline.dataimport.import_categories( \
        'data/arin/FlavinWhi5_3_ffcorr_small_OscillationEvals.txt')
#Whi5_dcategory = [3] * len(Whi5_rawdata)
Whi5_births = pipeline.dataimport.import_births( \
        'data/arin/FlavinWhi5_3_ffcorr_small_births.csv')

# Import information from Baumgartner et al. (2018)
Baum18_rawdata = pd.read_csv('data/baumgartner2018/Baumgartner2018_AllNormData.csv',
                             header = 0)
...
Baum18_dcategory0 = \
        pd.read_csv('data/baumgartner2018/Baumgartner2018_OscillationEvals.csv',
                    sep = ',', header = None)
...
for ii, cell in enumerate(Flavin.cells):
    cell.source = 'flavin'
    cell.medium.base = 'synthetic minimal'
...
for ii, cell in enumerate(Whi5.cells):
    cell.source = 'whi5'
    cell.medium.base = 'synthetic complete'
    cell.medium.nutrients = {'glucose': 2}
...
if False:
    fig01_listCellAttr = [Flavin.cells[154],
                         Whi5.cells[23]]
    fig01, axs01 = plt.subplots(ncols = len(fig01_listCellAttr), nrows = 1,
                                sharex = True, sharey = True,
                                gridspec_kw = {'hspace': 0.1})
    for ii, cell in enumerate(fig01_listCellAttr):
        axs01[ii].plot(cell.time, cell.flavin.reading_processed, color = '#3714b0')
        if cell.births.any():
...
if False:
    ...
    axs02[0].plot(cell.time, cell.flavin.reading, color = '#3714b0')
    if cell.births.any():
        for birth in cell.births:
            axs02[0].axvline(birth, ymin = 0, ymax = 1,
                          color = '#cb0077', linestyle = '--')
    axs02[0].set_ylabel('Flavin fluorescence (AU)')
    axs02[1].plot(cell.time, cell.mCherry.reading, color = '#6f0aaa')
    if cell.births.any():
        for birth in cell.births:
...
# good TS, its FT & AR; bad TS.  Spits out plots individually because I'll
# stitch them together in Inkscape anyway.

if True:
    goodcell = Flavin.cells[72]
    badcell = Flavin.cells[202]

    # good TS
    fig10, axs10 = plt.subplots()
    axs10.plot(goodcell.time, goodcell.flavin.reading_processed,
               color = '#3714b0')
    axs10.set_xlabel('Time (min)')
    axs10.set_ylabel('Flavin fluorescence (AU)')
    axs10.set_title('Good quality time series')
    plt.show()

    # good FT
    fig11, axs11 = plt.subplots()
    axs11.plot(goodcell.flavin.classical.freqs,
               goodcell.flavin.classical.power,
               color = '#725ac3')
    axs11.set_xlim([0, 0.02])
    axs11.set_ylim([0, 110])
    axs11.set_xlabel('Frequency ($min^{-1}$)')
    axs11.set_ylabel('Power (dimensionless)')
    axs11.set_title('Fourier spectrum')
    plt.show()
    ...
    def add_autoregAttr(cell):
        ...
        cell.flavin.autoreg.label = \
                'Autogressive Model-Based Periodogram'
        ...
        cell.flavin.autoreg.freqs, cell.flavin.autoreg.power = \
                pipeline.periodogram.autoreg(cell.time,
                                         cell.flavin.reading_processed,
                                         freq_npoints)
    add_autoregAttr(goodcell)
    fig12, axs12 = plt.subplots()
    axs12.plot(goodcell.flavin.autoreg.freqs,
               goodcell.flavin.autoreg.power,
               color = '#725ac3')
    axs12.set_xlim([0, 0.02])
    axs12.set_ylim([0, 14])
    axs12.set_xlabel('Frequency ($min^{-1}$)')
    axs12.set_ylabel('Power (dimensionless)')
    axs12.set_title('Autoregressive Model-Based Periodogram')
    plt.show()

    # bad TS
    fig13, axs13 = plt.subplots()
    axs13.plot(badcell.time, badcell.flavin.reading_processed,
               color = '#3714b0')
    axs13.set_xlabel('Time (min)')
    axs13.set_ylabel('Flavin fluorescence (AU)')
    axs13.set_title('Bad quality time series')
    plt.show()

    # bad FT
    fig14, axs14 = plt.subplots()
    axs14.plot(badcell.flavin.classical.freqs,
               badcell.flavin.classical.power,
               color = '#725ac3')
    axs14.set_xlim([0, 0.02])
    axs14.set_ylim([0, 110])
    axs14.set_xlabel('Frequency ($min^{-1}$)')
    axs14.set_ylabel('Power (dimensionless)')
    axs14.set_title('Fourier spectrum')
    plt.show()

    # bad AR
    add_autoregAttr(badcell)
    fig15, axs15 = plt.subplots()
    axs15.plot(badcell.flavin.autoreg.freqs,
               badcell.flavin.autoreg.power,
               color = '#725ac3')
    axs15.set_xlim([0, 0.02])
    axs15.set_ylim([0, 14])
    axs15.set_xlabel('Frequency ($min^{-1}$)')
    axs15.set_ylabel('Power (dimensionless)')
    axs15.set_title('Autoregressive Model-Based Periodogram')
    plt.show()
